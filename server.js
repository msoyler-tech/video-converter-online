...
// Bulut platformları (Render vb.) portu otomatik atar. Yerelde yine 3001 çalışır.
...
app.post('/convert/mp4', (req, res) => {
  ...
  ffmpeg(inputPath)
    ...
    .output(outputPath)
    .on('start', () => console.log('MP4 dönüştürme başladı...'))
    .on('end', () => {
...
app.post('/convert/yuv', (req, res) => {
  ...
  ffmpeg(inputPath)
    .outputOptions(['-f rawvideo', '-pix_fmt yuv420p'])
    .output(outputPath)
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`📍 Mod: ${process.env.NODE_ENV || 'Geliştirme'}`);
  console.log('='.repeat(50) + '\n');
});